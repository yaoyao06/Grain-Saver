...
<view class="order">
  ...
  <view class="mode" @tap="selectMode">
    <text class="text1">delivery method</text>
    <text class="text2">{{modes || "select a delivery method"}}</text>
...
<!--    <view class="morelist" style="border-bottom:none">
  ...
  <view class="order_more">
    ...
    <view class="morelist">
      <view class="title">
        <!-- <text class="quan" :style="'background:' + colors">send</text> -->
    ...
    <view class="morelist">
      <text class="title">Outstanding payment</text>
      <view class="right_title" :style="'color:' + colors + ';'">
        ￥{{sumprice}}
      </view>
    </view>
    <view class="tips">
      <view class="tips_name">Remark</view>
      <view class="textarea_box">
        <textarea placeholder="Please enter the remark" placeholder-class="font-size: 24upx" maxlength="-1" v-if="couponshow == false"></textarea>
      </view>
    </view>
  </view>
  <view class="bottom_btn">
    <view class="moneys">
    Total: <text :style="'color:' + colors + ';'">￥{{sumprice}}</text>
    </view>
  ...
  methods: {
	getSumPrice(){
		...
		this.goodsList.forEach(e=>{
			sumprice = (sumprice+Number(e.money)*e.number).toFixed(2)
		})
    ...
    submit() {
      //提交订单
      uni.navigateTo({
        url: '/pages/views/order/success?sumprice=' + this.sumprice
      });
    },
