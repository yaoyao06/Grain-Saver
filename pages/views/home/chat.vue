					...
					<view  class="flex"  style="width: 500rpx;">
						<view class="margin-left padding-chat flex-column-start" style="border-radius: 35rpx;background-color: #f9f9f9;">
							...
							<view class="flex-column-start" v-if="x.type==1" style="color: #2fa39b;">
								<text style="color: #838383;font-size: 22rpx;margin-top: 15rpx;">You can ask me like this:</text>
								...
								<view class="flex-row-start  padding-top-sm">
									<text class="my-neirong-sm">Don’t have the answer you want?</text>
									<text class="padding-left" style="color: #007AFF;">change</text>
								</view>
							</view>
							<!-- 消息模板 =>多个答案 -->
		...
		:animation="animationData" >		
			<view class="bottom-dh-char flex-row-around" style="font-size: 55rpx;">
				<!-- vue无法使用软键盘"发送" -->
				 ...
				 placeholder="Describe your problem in a short sentence" /> 
			...
			<view class="box-normal flex-row-around flex-wrap">
				<view class="tb-text">
					...
					<text >Problem feedback</text>
				</view>
				<view class="tb-text">
					<view class="cuIcon-form"></view>
					<text>Human customer service</text>
				</view>
	...
	export default {	
		...
		data() {
			return {
				...
				msgList:[{my:false,msg:"Hello, I am customer service robot Nana. How may I assist you today?",
				type:1,questionList:["How to cancel user account","I want to understand the business process","How to change phone number"]}],
				msg:"",
				go:0,
				srcollHeight:0
			}
			...
			msgKf(x){				
				...
				if(this.msg=='hello'){
					setTimeout(()=>{
						// 取消loading
						this.msgLoad=false
						this.msgList.push({my:false,msg:"Hello, do you need any assistance?"})
						this.msgGo()
					},2000)
				}else if(this.msg=='Carbon account level'){
					setTimeout(()=>{
						// 取消loading
						this.msgLoad=false
						this.msgList.push({my:false,msg:"Hi, here is a translation of the information related to Carbon account levels:\nYour current account level is Lv3. Carbon Account Level: lv1\nPurchase discount: None, Loan limit: 2000.00 RMB, Loan interest rate per year: 5.65%\nCarbon account level: lv2\nPurchase discount: 4% off, Loan limit: 4000.00 RMB, Loan interest rate per year: 5.50%\nCarbon account level: lv3\nPurchase discount: 7% off, Loan limit: 6000.00 RMB, Loan interest rate per year: 5.35%\nCarbon account level: lv4\nPurchase discount: 9% off, Loan limit: 8000.00 RMB, Loan interest rate per year: 5.20%\nCarbon account level: lv5\nPurchase discount: 11% off, Loan limit: 10000.00 RMB, Loan interest rate per year: 5.05%\nCarbon account level: lv6\nPurchase discount: 13% off, Loan limit: 12000.00 RMB, Loan interest rate per year: 4.90%\nCarbon account level: lv7\nPurchase discount: 15% off, Loan limit: 15000.00 RMB, Loan interest rate per year: 4.75%."})
						this.msgGo()
					},2000)
				}else{
					setTimeout(()=>{
						// 取消loading
						this.msgLoad=false
						this.msgList.push({my:false,msg:"娜娜还在学习中,没能明白您的问题"})
						this.msgGo()
					},2000)
				}
				// setTimeout(()=>{
				// 	// 取消loading
				// 	this.msgLoad=false
				// 	this.msgList.push({my:false,msg:"你好，请问有什么需要帮助的"})
